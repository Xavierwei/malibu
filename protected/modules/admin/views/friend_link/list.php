<div class="content">
    <?php $form=$this->beginWidget('CActiveForm',array(
        'id'=>'listForm',
        'action'=>array('/admin/friend_link/auditAll/'),
    ));?>
    <table class="contentTab listTab" width="100%">
        <tr>
            <td class="titleTd" colspan="9">友情链接</td>
        </tr>
        <?php
        ...
        <?php foreach($data as $key=>$item){ ?>
            <tr>
                ...
                <td><?php echo CHtml::link($item->name,array('/admin/friend_link/edit/','id'=>$item->id)); ?></td>
                ...
                <td><?php echo CHtml::link($audit,array('/admin/friend_link/audit/','id'=>$item->id)); ?></td>
                <td><?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/edit.gif">',array('/admin/friend_link/edit/','id'=>$item->id)); ?> <?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/del.gif">',array('/admin/friend_link/delete/','id'=>$item->id),array('class'=>'delete','id'=>$item->id)); ?></td>
            </tr>
        <?php }?>
        <tr>
            <td class="pageTd" colspan="9">
                <div class="action">
                    ...
                    <?php echo CHtml::Button('未审核',array('class'=>'button','onclick'=>'formSubmit("'.$this->createUrl('/admin/friend_link/unAuditAll/').'","")'));?>
                    <?php echo CHtml::Button('删除',array('class'=>'button','onclick'=>'formSubmit("'.$this->createUrl('/admin/friend_link/deleteAll/').'","确定要删除吗？")'));?>
                </div>
                <?php    
