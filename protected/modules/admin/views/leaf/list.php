		...
		<?php foreach($data as $key=>$item){ ?>
			<tr>
				...
				<td width="10">
                    ...
                    if($key>0&&($item->lft!=$temp+1)){echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/up.gif">',array('/admin/leaf/moveUp/','id'=>$item->id));}
					?>
				</td>
				<td width="10">
					<?php
						$temp=isset($item->parent_node)?$item->parent_node->rgt:0;
						if($item->id!=$item->last_brother->id&&($item->rgt!=$temp-1)){echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/down.gif">',array('/admin/leaf/moveDown/','id'=>$item->id));}
					?>
				</td>
				...
				<td><?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/edit.gif">',array('/admin/leaf/edit/','id'=>$item->id)); ?> <?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/del.gif">',array('/admin/leaf/delete/','id'=>$item->id),array('class'=>'delete','id'=>$item->id)); ?></td>
			</tr>
		<?php }?>
		<tr>
