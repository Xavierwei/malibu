<div class="content">
	<?php $form=$this->beginWidget('CActiveForm',array(
		'id'=>'listForm',
	    'action'=>array('/admin/category/auditAll/'),
	));?>
	<table class="contentTab listTab" width="100%">
		...
		<?php foreach($data as $key=>$item){ ?>
			<tr>
				...
				<td><?php echo $item->space;?><?php echo CHtml::link($item->name,array('/admin/category/edit/','id'=>$item->id)); ?></td>
				...
				<td width="10">
					<?php 
						...
						if($key>0&&($item->lft!=$temp+1)){echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/up.gif">',array('/admin/category/moveUp/','id'=>$item->id));}
					?>
				</td>
				<td width="10">
					<?php
						$temp=isset($item->parent_node)?$item->parent_node->rgt:0;
						if($item->id!=$item->last_brother->id&&($item->rgt!=$temp-1)){echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/down.gif">',array('/admin/category/moveDown/','id'=>$item->id));}
					?>
				</td>
				...
				<td><?php echo CHtml::link($audit,array('/admin/category/audit/','id'=>$item->id)); ?></td>
				<td><?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/edit.gif">',array('/admin/category/edit/','id'=>$item->id)); ?> <?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/del.gif">',array('/admin/category/delete/','id'=>$item->id),array('class'=>'delete','id'=>$item->id)); ?></td>
			</tr>
		<?php }?>
		<tr>
			<td class="pageTd" colspan="11">
				<div class="action">
					...
					<?php echo CHtml::Button('未审核',array('class'=>'button','onclick'=>'formSubmit("'.$this->createUrl('/admin/category/unAuditAll/').'","")'));?>
				</div>
			</td>
		</tr>
	</table>
