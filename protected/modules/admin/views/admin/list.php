<div class="content">
	<?php $form=$this->beginWidget('CActiveForm',array(
		'id'=>'listForm',
	    'action'=>array('/admin/admin/auditAll/'),
	));?>
	<table class="contentTab listTab" width="100%">
		<tr>
			<td class="titleTd" colspan="11">管理员</td>
		</tr>
		<?php
		...
		<?php foreach($data as $key=>$item){ ?>
			<tr>
				...
				<td><?php echo CHtml::link($item->name,array('/admin/admin/edit/','id'=>$item->id)); ?></td>
				...
				<td><?php echo CHtml::link($audit,array('/admin/admin/audit/','id'=>$item->id)); ?></td>
				<td>
				<?php 
					if(Yii::app()->user->id==1){
						echo $item->id==1?CHtml::link('重置权限',array('/admin/authority/reset/')):CHtml::link('权限分配',array('/admin/authority/edit/','id'=>$item->id));
					}else{
						echo $item->id==1?'':CHtml::link('权限分配',array('/admin/authority/edit/','id'=>$item->id));
					}
				?>
				</td>
				<td><?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/edit.gif">',array('/admin/admin/edit/','id'=>$item->id)); ?> <?php echo CHtml::link('<img src="'.yii::app()->baseUrl.'/style/admin/images/del.gif">',array('/admin/admin/delete/','id'=>$item->id),array('class'=>'delete','id'=>$item->id)); ?></td>
			</tr>
		<?php }?>
		<tr>
			<td class="pageTd" colspan="11">
				<div class="action">
					...
					<?php echo CHtml::Button('未审核',array('class'=>'button','onclick'=>'formSubmit("'.$this->createUrl('/admin/admin/unAuditAll/').'","")'));?>
					<?php echo CHtml::Button('删除',array('class'=>'button','onclick'=>'formSubmit("'.$this->createUrl('/admin/admin/deleteAll/').'","确定要删除吗？")'));?>
				</div>
				<?php    
