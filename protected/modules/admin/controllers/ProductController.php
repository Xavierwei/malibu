...
class ProductController extends Controller
{
	...
	public function actionList()
	{
        ...
        $this->render('list',array('data'=>$data,'page'=>$pager,'model'=>Product::model()));
	}

	public function actionCreat()
	{
		$model = new Product();
		if(Yii::app()->request->getParam('Product'))
		{
	...
	public function actionAuditAll()
	{
		if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
			...
			$count = Product::model()->updateAll(array("audit"=>1)," `id` in ('".$id."')");
			if($count){
				Yii::app()->user->setFlash('submit','审核提交成功！');
			}else{
				Yii::app()->user->setFlash('submit','审核提交失败！');
	...
	public function actionUnAuditAll()
	{
		if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
			$id = implode("','",Yii::app()->request->getParam('id'));
			$count = Product::model()->updateAll(array("audit"=>0)," `id` in ('".$id."')");
			if($count){
				Yii::app()->user->setFlash('submit','审核提交成功！');
			}else{
				Yii::app()->user->setFlash('submit','审核提交失败！');
	...
	public function loadModel()
	{
		if($this->_model===null)
		{
			...
			{
				$this->_model=Product::model()->findByPk(Yii::app()->request->getParam('id'));
			}
			if($this->_model===null){
