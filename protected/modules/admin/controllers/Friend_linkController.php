...
class Friend_linkController extends Controller
{
    ...
    public function actionList()
    {
        ...
        $criteria->with = 'menu';
        $count = Friend_link::model()->count($criteria);
        $pager = new CPagination($count);
        $pager->pageSize = 20;      
        $pager->applyLimit($criteria);
        $data = Friend_link::model()->findAll($criteria);
        $this->render('list',array('data'=>$data,'page'=>$pager,'model'=>Friend_link::model()));
    }

    public function actionCreat()
    {
        $model = new Friend_link();
        if(Yii::app()->request->getParam('Friend_link'))
        {
            $model->attributes=Yii::app()->request->getParam('Friend_link');
            if($model->save()){
    ...
    public function actionAuditAll()
    {
        if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
            ...
            $count = Friend_link::model()->updateAll(array("audit"=>1)," `id` in ('".$id."')");
            if($count){
                Yii::app()->user->setFlash('submit','审核提交成功！');
            }else{
                Yii::app()->user->setFlash('submit','审核提交失败！');
    ...
    public function actionUnAuditAll()
    {
        if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
            $id = implode("','",Yii::app()->request->getParam('id'));
            $count = Friend_link::model()->updateAll(array("audit"=>0)," `id` in ('".$id."')");
            if($count){
                Yii::app()->user->setFlash('submit','审核提交成功！');
            }else{
                Yii::app()->user->setFlash('submit','审核提交失败！');
    ...
    {
        if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
            $count = Friend_link::model()->deleteByPk(Yii::app()->request->getParam('id'));
            if($count){
                Yii::app()->user->setFlash('submit','删除提交成功！');
            }else{
    ...
    public function loadModel()
    {
        if($this->_model===null)
        {
            ...
            {
                $this->_model=Friend_link::model()->findByPk(Yii::app()->request->getParam('id'));
            }
            if($this->_model===null){
