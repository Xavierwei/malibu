...
class AdvertisementController extends Controller
{
    ...
    public function actionList()
    {
        ...
        $criteria->with = 'advertising';
        $count = Advertisement::model()->count($criteria);
        $pager = new CPagination($count);
        $pager->pageSize = 20;      
        $pager->applyLimit($criteria);
        $data = Advertisement::model()->findAll($criteria);
        $this->render('list',array('data'=>$data,'page'=>$pager,'model'=>Advertisement::model()));
    }

    public function actionCreat()
    {
        $model = new Advertisement();
        if(Yii::app()->request->getParam('Advertisement'))
        {
            $model->attributes=Yii::app()->request->getParam('Advertisement');
            if($model->save()){
    ...
    public function actionAuditAll()
    {
        if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
            ...
            $count = Advertisement::model()->updateAll(array("audit"=>1)," `id` in ('".$id."')");
            if($count){
                Yii::app()->user->setFlash('submit','审核提交成功！');
            }else{
                Yii::app()->user->setFlash('submit','审核提交失败！');
    ...
    public function actionUnAuditAll()
    {
        if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
            $id = implode("','",Yii::app()->request->getParam('id'));
            $count = Advertisement::model()->updateAll(array("audit"=>0)," `id` in ('".$id."')");
            if($count){
                Yii::app()->user->setFlash('submit','审核提交成功！');
            }else{
                Yii::app()->user->setFlash('submit','审核提交失败！');
    ...
    {
        if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
            $count = Advertisement::model()->deleteByPk(Yii::app()->request->getParam('id'));
            if($count){
                Yii::app()->user->setFlash('submit','删除提交成功！');
            }else{
    ...
    public function loadModel()
    {
        if($this->_model===null)
        {
            ...
            {
                $this->_model=Advertisement::model()->findByPk(Yii::app()->request->getParam('id'));
            }
            if($this->_model===null){
