...
class ArticleController extends Controller
{
	...
	public function actionList()
	{
        ...
        $this->render('list',array('data'=>$data,'page'=>$pager,'model'=>Article::model()));
	}

	public function actionCreat()
	{
		$model = new Article();
		if(Yii::app()->request->getParam('Article'))
		{
			$model->attributes=Yii::app()->request->getParam('Article');
			if($model->save()){
	...
	public function actionAuditAll()
	{
		if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
			...
			$count = Article::model()->updateAll(array("audit"=>1)," `id` in ('".$id."')");
			if($count){
				Yii::app()->user->setFlash('submit','审核提交成功！');
			}else{
				Yii::app()->user->setFlash('submit','审核提交失败！');
	...
	public function actionUnAuditAll()
	{
		if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
			$id = implode("','",Yii::app()->request->getParam('id'));
			$count = Article::model()->updateAll(array("audit"=>0)," `id` in ('".$id."')");
			if($count){
				Yii::app()->user->setFlash('submit','审核提交成功！');
			}else{
				Yii::app()->user->setFlash('submit','审核提交失败！');
	...
	{
		if(Yii::app()->request->getParam('id')&&is_array(Yii::app()->request->getParam('id'))){
			$count = Article::model()->deleteByPk(Yii::app()->request->getParam('id'));
			if($count){
				Yii::app()->user->setFlash('submit','删除提交成功！');
			}else{
	...
	public function loadModel()
	{
		if($this->_model===null)
		{
			...
			{
				$this->_model=Article::model()->findByPk(Yii::app()->request->getParam('id'));
			}
			if($this->_model===null){
